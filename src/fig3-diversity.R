...
plants %>%
  filter(taxon %in% df1$Forests & ! taxon %in% df1$Meadows & ! taxon %in% df1$`Urban habitats`) %>%
  group_by(taxon) %>%
  tally() %>%
  arrange(-n)
...
fungi %>%
  filter(ASV %in% df2$Forests & ! ASV %in% df2$Meadows & ! ASV %in% df2$`Urban habitats`) %>% 
...
bacteria %>%
  filter(ASV %in% df3$Forests & ! ASV %in% df3$Meadows & ! ASV %in% df3$`Urban habitats`) %>% 
  filter(species != "unidentified" & !is.na(species)) %>%
  select(species, id) %>%
  unique %>%
  group_by(species) %>%
...
read.csv("data/soils.csv", fileEncoding = "latiforest") -> soils
